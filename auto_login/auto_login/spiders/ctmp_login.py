...
class CtmpLoginSpider(RedisSpider):
    """
    优先登录生意参谋，不关闭浏览器继续登录赤兔名品
    """
    name = "ctmp_login"
    ...
    def parse(self, response):
        ...
        try:
                ...
                if "member/login_unusual.htm" in judge_url:
                    ...
                    while time_counter:
                        ...
                        if sns_code:
                            ...
                            submit_btn_2 = self.wait.until(EC.presence_of_element_located((By.ID,
                                                                                           'submitBtn')))
                            submit_btn_2.click()
                            login_ok = True

                if login_ok:
                    self.driver.get('https://newkf.topchitu.com/')
                    time.sleep(3)
                    use_btn = self.wait.until(EC.presence_of_element_located((By.CLASS_NAME, 'put-middle')))
                    use_btn.click()
                    time.sleep(3)
                    auth_sub_btn = self.wait.until(EC.presence_of_element_located((By.ID, 'sub')))
                    auth_sub_btn.click()
                    time.sleep(3)
            ...
            import traceback
            self.logger.warn("赤兔名品自动登录异常={}", ex)
            self.logger.warn("赤兔名品自动登录异常={}".format(traceback.format_exc()))
        finally:
    ...
    def send_alert(self, account_name):
        ...
        try:
            ...
            import traceback
            self.logger.error("-> send_alert ", ex)
